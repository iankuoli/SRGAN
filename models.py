...
class VGGNetwork:
    ...
    def append_vgg_network(self, x_in, true_X_input, pre_train=False):
        ...
        x = Conv2D(64, (3, 3), activation='relu', name='vgg_conv1_1', border_mode='same')(x)
        ...
        if pre_train:
            ...
            x = Conv2D(128, (3, 3), activation='relu', name='vgg_conv2_2', border_mode='same',
        ...
        if not pre_train:
            ...
            x = Conv2D(512, (3, 3), activation='relu', name='vgg_conv5_3', border_mode='same',
                       activity_regularizer=vgg_regularizer5)(x)
        else:
            x = Conv2D(512, (3, 3), activation='relu', name='vgg_conv5_3', border_mode='same')(x)
...
class DiscriminatorNetwork:
    ...
    def append_gan_network(self, true_X_input):
        ...
        x = Conv2D(64, (self.k, self.k), border_mode='same', name='gan_conv1_1')(x)
        x = LeakyReLU(0.3, name="gan_lrelu1_1")(x)

        x = Conv2D(64, (self.k, self.k), border_mode='same', name='gan_conv1_2', subsample=(2, 2))(x)
        ...
        for i, nb_filters in enumerate(filters):
            for j in range(2):
                ...
                x = Conv2D(nb_filters, (self.k, self.k), border_mode='same', subsample=subsample,
    ...
    def load_gan_weights(self, model):
        ...
        if self.gan_layers is None:
            self.gan_layers = [layer for layer in model.layers if 'gan_' in layer.name]
...
class GenerativeNetwork:
    ...
    def create_sr_model(self, ip):
        ...
        x = Conv2D(self.filters, (5, 5), activation='linear', border_mode='same', name='sr_res_conv1',
                   init=self.init)(ip)
        ...
        x = Conv2D(3, (5, 5), activation='tanh', border_mode='same', activity_regularizer=tv_regularizer,
    ...
    def _residual_block(self, ip, id):
        ...
        x = Conv2D(self.filters, (3, 3), activation='linear', border_mode='same', name='sr_res_conv_' + str(id) + '_1',
                   init=self.init)(ip)
        ...
        x = Conv2D(self.filters, (3, 3), activation='linear', border_mode='same', name='sr_res_conv_' + str(id) + '_2',
                   init=self.init)(x)
    ...
    def _upscale_block(self, ip, id):
        ...
        x = Conv2D(128, (3, 3), activation="linear", border_mode='same', name='sr_res_upconv1_%d' % id,
                   init=self.init)(init)
        x = LeakyReLU(alpha=0.25, name='sr_res_up_lr_%d_1_1' % id)(x)
        x = UpSampling2D(name='sr_res_upscale_%d' % id, size=(2, 2))(x)
        #x = SubPixelUpscaling(r=2, channels=32)(x)
        x = Conv2D(128, (3, 3), activation="linear", border_mode='same', name='sr_res_filter1_%d' % id,
                   init=self.init)(x)
...
class SRGANNetwork:
    ...
    def __init__(self, img_width=96, img_height=96, batch_size=16, nb_scales=2):
        ...
        # The * _network objects: refer to the helper classes which create and manage the Keras models,
        #                         load and save weights and set whether the model can be trained or not.
        #
        ...
        self.srgan_model_ = None  # type: Model which can be: srgan_pretrain_model, discriminator_pretrain_model, full
    ...
    def build_srgan_pretrain_model(self, use_small_srgan=False):
        #
        # X_LR ==> [SR Generator] ==> X_SR
        # X_SR, X_HR ==> [VGG] ==> vgg_output (feature_map)
        #
        ...
        # ----- Input Declaration ----- #
        #
        ip = Input(shape=(3, self.img_width, self.img_height), name='x_generator')  # (X_LR)
        ip_vgg = Input(shape=(3, large_width, large_height), name='x_vgg')          # Actual X images (X_HR)

        #
        # ----- Individual Model Declaration ----- #
        #
        sr_output = self.generative_network.create_sr_model(ip)  # X with SR
        self.generative_model_ = Model(input=ip, output=sr_output)

        #
        # ----- Whole Model Integration ----- #
        #
        vgg_output = self.vgg_network.append_vgg_network(sr_output, ip_vgg, pre_train=True)
        self.srgan_model_ = Model(input=[ip, ip_vgg], output=vgg_output)
    ...
    def build_discriminator_pretrain_model(self, use_small_srgan=False, use_small_discriminator=False):
        #
        # X_LR ==> [SR Generator] ==> X_SR
        # X_SR, X_HR ==> [Discriminator] ==> gan_output (binary category)
        #
        ...
        # ----- Input Declaration ----- #
        #
        ip = Input(shape=(3, self.img_width, self.img_height), name='x_generator')    # X with LR
        ip_gan = Input(shape=(3, large_width, large_height), name='x_discriminator')  # Actual X images (X_SR or X_HR)
        ...
        self.discriminative_model_ = Model(input=ip_gan, output=gan_output)
    ...
    def build_srgan_model(self, use_small_srgan=False, use_small_discriminator=False):
        ...
        # ----- Input Declaration ----- #
        #
        ip = Input(shape=(self.img_width, self.img_height, 3), name='x_generator')    # (X_LR)
        ip_gan = Input(shape=(large_width, large_height, 3), name='x_discriminator')  # Actual X images (X_SR or X_HR)
        ip_vgg = Input(shape=(large_width, large_height, 3), name='x_vgg')            # Actual X images (X_HR)
